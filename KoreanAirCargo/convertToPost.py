...
def KoreanEvent(event):
    if(event.find("Booked") != -1):
        ...
        return ('ARR', "Arrived")
    elif(event.find("Consignee Notified") != -1):
...
def KoreanPost(step):
    ...
    postJson["resolvedEventSource"] = "Korean RPA"
    ...
    postJson["eventCode"], postJson["eventName"] = KoreanEvent(data.get("Event"))
    postJson["carrierName"] = data.get("Air Carrier")
    if(postJson["eventCode"] == None):
        return
    dt = datetime.datetime.strptime(data.get("Time").strip(), "%d %b%H:%M")
